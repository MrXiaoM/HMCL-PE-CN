...
public class World {
    ...
    private void loadFromZip() throws IOException {
        ...
        com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.deleteDirectory(r);
    ...
    public void install(Path savesDir, String name) throws IOException {
        ...
        if (Files.isRegularFile(file)) {
            com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.deleteDirectory(AppManifest.SAVES_CACHE_DIR + "/install/world");
            ...
            if (Files.isRegularFile(cur)) {
                com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.rename(AppManifest.SAVES_CACHE_DIR + "/install/world",name);
                com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.copyDirectory(AppManifest.SAVES_CACHE_DIR + "/install/" + name,worldDir.toString());
            } else {
                try (Stream<Path> stream = Files.list(new File(AppManifest.SAVES_CACHE_DIR + "/install/world/").toPath())) {
                    ...
                    com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.rename(AppManifest.SAVES_CACHE_DIR + "/install/world/" + subDirectoryName,name);
                    com.tungsten.hmclpe.sweetrice.utils.file.FileUtils.copyDirectory(AppManifest.SAVES_CACHE_DIR + "/install/world/" + name,worldDir.toString());
                }
            }
